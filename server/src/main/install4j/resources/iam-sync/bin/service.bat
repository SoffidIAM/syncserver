...
rem
rem Options
rem install                Install the service using SyncServer as service name.
...
IF /i %PROCESSOR_ARCHITECTURE% EQU X86 SET "proc=%~dp0SyncServer.x86.exe"
IF /i %PROCESSOR_ARCHITECTURE% EQU AMD64 SET "proc=%~dp0SyncServer.amd64.exe"
IF /i %PROCESSOR_ARCHITECTURE% EQU IA64 SET "proc=%~dp0SyncServer.ia64.exe"
...
rem Set default Service name (If you change this then rename also SyncServer.exe to the same name)
set SERVICE_NAME=Soffid-iamsync
set PR_DISPLAYNAME=Soffid 3 IAM Sync
...
set SERVICE_NAME=%1
set PR_DISPLAYNAME=Soffid 3 IAM Sync (%1)
shift
goto checkUser
:runAsUser
shift
...
set "PR_CLASSPATH=%CATALINA_HOME%\bin\bootstrap.jar;%CATALINA_BASE%\lib\mariadb-java-client-1.8.0.jar;%CATALINA_BASE%\lib\ojdbc10-19.20.0.0.jar;%CATALINA_BASE%\lib\postgresql-42.2.5.jre7.jar;%CATALINA_BASE%\lib\mssql-jdbc-12.10.0.jre11.jar"
...
"%EXECUTABLE%" //IS//%SERVICE_NAME% ^
    ...
    --StartClass com.soffid.iam.sync.bootstrap.SyncLoader ^
    --Startup auto ^
...
echo Installed, will now configure Sync Server
...
"%EXECUTABLE%" //US//%SERVICE_NAME% ^
	++JvmOptions "-Djava.awt.headless=true;-XX:+UseParallelGC;-XX:MaxPermSize=256M"
