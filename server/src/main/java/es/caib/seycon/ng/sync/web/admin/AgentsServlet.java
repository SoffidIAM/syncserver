    ...
    private String getTasquesPendentsAgent(String agent) throws InternalErrorException {
        ...
        for (DispatcherHandler dispatcher : taskGenerator.getDispatchers()) {
            if (dispatcher != null && dispatcher.getDispatcher() != null
                    && agent.equals(dispatcher.getDispatcher().getCodi())) {
                toReturn.append("<P>URL: ");
                toReturn.append(dispatcher.getDispatcher().getUrl());
                toReturn.append("</p>");
    ...
    private String getTasquesPendentsAgents(boolean mostraCertificats) throws IOException, InternalErrorException {
        ...
        for (DispatcherHandler taskDispatcher : taskGenerator.getDispatchers()) {
            if (taskDispatcher.isActive()) {
                toReturn.append("<tr>");
                toReturn.append("<td> <a href=agents?agent=" + URLEncoder.encode(taskDispatcher.getDispatcher().getCodi(), "UTF-8") + " >");
                toReturn.append(taskDispatcher.getDispatcher().getCodi());
                ...
                toReturn.append("</td>");
                toReturn.append("<td>" + taskDispatcher.getDispatcher().getNomCla() + "</td>");
