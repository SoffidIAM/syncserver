...
import es.caib.seycon.ng.sync.engine.cert.CertificateManager;
import es.caib.seycon.ng.sync.engine.cert.ValidadorFactory;
...
public class CertificateLoginServlet extends HttpServlet {
    ...
    Usuari validateUser() throws UnknownUserException, InternalErrorException,
        ...
        Usuari ui = s.getUserInfo(new X509Certificate[] {userCertificate});
        ...
        String dbLlinatge1 = p.normalizeName(ui.getPrimerLlinatge());
        String dbLlinatge2 = ui.getSegonLlinatge() == null ? "" : p.normalizeName(ui.getSegonLlinatge());

        if (!dbNom.equals(certNom) 
                || !dbLlinatge1.equals(certLlinatge1)
                || !dbLlinatge2.equals(certLlinatge2)) {
            // Segon intent
            String dbLlinatges [] = p.parse(ui.getPrimerLlinatge()+" "+ui.getSegonLlinatge(), 2);
            if (!dbNom.equals(certNom) 
