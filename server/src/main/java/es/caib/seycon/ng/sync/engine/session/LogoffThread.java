...
public class LogoffThread extends Object implements Runnable {
    Collection<Sessio> sessionInfoVector;
    private SessionManager sessionManager;

    /**
     * Constructor
     */
    public LogoffThread(Collection<Sessio> sessionList, SessionManager sm) {
    ...
    public void run() {
        ...
        while (!salir) {
            synchronized (sessionInfoVector) {
                Iterator<Sessio> it = sessionInfoVector.iterator();
                if (! it.hasNext())
                    salir = true;
                else {
                    s = it.next();
            ...
            if (!salir) {
                try {
                	Long lastPing = s.getDataKeepAlive() == null ? 
                						s.getDataInici().getTimeInMillis():
                						s.getDataKeepAlive().getTimeInMillis();
