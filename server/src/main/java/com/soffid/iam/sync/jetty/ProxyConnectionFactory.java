 ...
 * </p>
 * <p>
 * This factory can be placed in front of any other connection factory to
 * process the proxy v1 or v2 line before the normal protocol handling
 * </p>
 *
 * @see <a href=
 *      "http://www.haproxy.org/download/1.5/doc/proxy-protocol.txt">http://www.haproxy.org/download/1.5/doc/proxy-protocol.txt</a>
 */
public class ProxyConnectionFactory extends DetectorConnectionFactory {
	...
	public ProxyConnectionFactory(String nextProtocol) {
		super(new ProxyV1ConnectionFactory(nextProtocol, null), new ProxyV2ConnectionFactory(nextProtocol, null));
	}

	public ProxyConnectionFactory(String nextProtocol, String trustedProxies) throws UnknownHostException {
		super(new ProxyV1ConnectionFactory(nextProtocol, new NetmaskMatcher(trustedProxies)), 
				new ProxyV2ConnectionFactory(nextProtocol, new NetmaskMatcher(trustedProxies)));
	}

	private static ConnectionFactory findNextConnectionFactory(String nextProtocol, Connector connector,
			String currentProtocol, EndPoint endp) {
		...
		if (nextProtocolToFind == null)
			throw new IllegalStateException("Cannot find protocol following '" + currentProtocol
					+ "' in connector's protocol list " + connector.getProtocols() + " for " + endp);
		ConnectionFactory connectionFactory = connector.getConnectionFactory(nextProtocolToFind);
		if (connectionFactory == null)
			throw new IllegalStateException("Cannot find protocol '" + nextProtocol + "' in connector's protocol list "
					+ connector.getProtocols() + " for " + endp);
		if (LOG.isDebugEnabled())
	...
	private static class ProxyV1ConnectionFactory extends AbstractConnectionFactory implements Detecting {
		...
		private NetmaskMatcher trustedProxies;

		private ProxyV1ConnectionFactory(String nextProtocol, NetmaskMatcher trustedProxies) {
			super("proxy");
		...
		public Connection newConnection(Connector connector, EndPoint endp) {
			ConnectionFactory nextConnectionFactory = findNextConnectionFactory(_nextProtocol, connector, getProtocol(),
					endp);
			return configure(new ProxyProtocolV1Connection(endp, connector, nextConnectionFactory, trustedProxies), connector, endp);
		}
		...
		private static class ProxyProtocolV1Connection extends AbstractConnection
			...
			private int _index;
			private int _length;
			private NetmaskMatcher _trustedProxies;

			private ProxyProtocolV1Connection(EndPoint endp, Connector connector, ConnectionFactory next, NetmaskMatcher trustedProxies) {
			...
			private void upgrade() {
				...
				if (_trustedProxies != null && !_trustedProxies.match(getEndPoint().getRemoteSocketAddress())) {
					proxyEndPoint = getEndPoint();
				}
				else if (unknown) {
					EndPoint endPoint = getEndPoint();
					proxyEndPoint = new ProxyEndPoint(endPoint, endPoint.getLocalSocketAddress(),
	...
	private static class ProxyV2ConnectionFactory extends AbstractConnectionFactory implements Detecting {
		...
		private static final byte[] SIGNATURE = new byte[] { 0x0D, 0x0A, 0x0D, 0x0A, 0x00, 0x0D, 0x0A, 0x51, 0x55, 0x49,
				0x54, 0x0A };
		...
		private ProxyV2ConnectionFactory(String nextProtocol, NetmaskMatcher trustedProxies) {
			super("proxy");
			this._nextProtocol = nextProtocol;
		...
		public Connection newConnection(Connector connector, EndPoint endp) {
			ConnectionFactory nextConnectionFactory = findNextConnectionFactory(_nextProtocol, connector, getProtocol(),
					endp);
			return configure(new ProxyProtocolV2Connection(endp, connector, nextConnectionFactory, _trustedProxies), connector, endp);
		}
		...
		private class ProxyProtocolV2Connection extends AbstractConnection
			...
			protected ProxyProtocolV2Connection(EndPoint endp, Connector connector, ConnectionFactory next, NetmaskMatcher _trustedProxies) {
				super(endp, connector.getExecutor());
				_connector = connector;
				_next = next;
				_buffer = _connector.getByteBufferPool().acquire(getInputBufferSize(), true);
				this._trustedProxies = _trustedProxies;
			}
			...
			public void onFillable() {
				try {
					...
					if (LOG.isDebugEnabled())
						LOG.debug("Proxy v2 onFillable header parsed, length = {}, buffer = {}", _length,
			...
			private void parseBodyAndUpgrade() throws IOException {
				...
				if (LOG.isDebugEnabled())
					LOG.debug("Proxy v2 parsing body, length = {}, buffer = {}", _length,
							BufferUtil.toHexSummary(_buffer));
				...
				if (_local) {
					...
					while (_buffer.remaining() > nonProxyRemaining) {
						...
						if (LOG.isDebugEnabled())
							LOG.debug(String.format("Proxy v2 T=%x L=%d V=%s for %s", type, length,
									TypeUtil.toHexString(value), this));
				...
				if (_trustedProxies != null && !_trustedProxies.match(getEndPoint().getRemoteSocketAddress())) {
					if (LOG.isDebugEnabled())
						LOG.debug("Proxy is not trusted, upgrading to {}", _nextProtocol);
			...
			private void parseHeader() throws IOException {
				if (LOG.isDebugEnabled())
					LOG.debug("Proxy v2 parsing fixed length packet part, buffer = {}",
							BufferUtil.toDetailString(_buffer));
				...
				if (!_local && (_family == Family.UNSPEC || transport != Transport.STREAM))
					throw new IOException(String.format("Proxy v2 unsupported PROXY mode 0x%x,0x%x", versionAndCommand,
							transportAndFamily));

				if (_length > getMaxProxyHeader())
					throw new IOException(String.format("Proxy v2 Unsupported PROXY mode 0x%x,0x%x,0x%x",
	...
	public static class ProxyEndPoint extends AttributesMap implements EndPoint, EndPoint.Wrapper {
		...
		public String toString() {
			return String.format("%s@%x[remote=%s,local=%s,endpoint=%s]", getClass().getSimpleName(), hashCode(),
					_remote, _local, _endPoint);
		}
